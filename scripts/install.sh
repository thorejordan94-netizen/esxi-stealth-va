...
if command -v testssl.sh >/dev/null 2>&1 || command -v testssl >/dev/null 2>&1; then
...
elif [ "$OFFLINE_MODE" = true ] && [ -x "./bin/testssl.sh" ]; then
    sudo cp ./bin/testssl.sh /usr/local/bin/testssl.sh
    sudo chmod +x /usr/local/bin/testssl.sh
elif [ "$OFFLINE_MODE" = false ] && [ ! -d "/usr/local/testssl.sh" ]; then
    if [ -n "$PROXY_URL" ]; then
    ...
    echo "[!] testssl.sh unavailable in offline mode; Python TLS checks remain enabled."
fi
