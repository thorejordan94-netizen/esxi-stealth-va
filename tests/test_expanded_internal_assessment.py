...
class ExpandedDiscoveryTests(unittest.TestCase):
    ...
    def test_broad_private_scope_is_split_without_losing_coverage(self):
        config = {"assessment": {"expanded_discovery": {
            "max_addresses_per_subnet": 65536,
            "max_addresses_per_sweep": 4096,
        }}}
        network = self.phase._validate_private_subnet("10.20.0.0/16", config)
        chunks = self.phase._scope_chunks(network, config)
        self.assertEqual(16, len(chunks))
        self.assertEqual("10.20.0.0/20", str(chunks[0]))
        self.assertEqual("10.20.240.0/20", str(chunks[-1]))
