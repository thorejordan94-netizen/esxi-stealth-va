...
# Quick scan (faster, top 100 ports)
...
### System
- Linux with `iproute2` (openSUSE/SLES and Ubuntu are supported)
- Python 3.6+
...
All enabled-phase tools are installed automatically when root or non-interactive
`sudo` and package/download access are available. Python TLS checks remain as a
fallback when `testssl.sh` cannot be installed.
...
|---------|------------------|---------------|
| Quick | 15-30 min | Top 100 |
| Standard | 30-60 min | Top 1,000 |
