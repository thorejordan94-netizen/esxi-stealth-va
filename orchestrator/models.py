...
class AssessmentReport:
    ...
    def add_host(self, host_finding: HostFinding):
        """Add or merge an infrastructure host finding by address."""
        existing = next((item for item in self.findings_infrastructure if item.host == host_finding.host), None)
        if existing is None:
            self.findings_infrastructure.append(host_finding)
            return
        if host_finding.hostname and not existing.hostname:
            existing.hostname = host_finding.hostname
        if host_finding.os_fingerprint and not existing.os_fingerprint:
            existing.os_fingerprint = host_finding.os_fingerprint
        if host_finding.role == "esxi_host" or not existing.role:
            existing.role = host_finding.role
        ...
        for port in host_finding.ports:
            ...
            if current is None:
                ...
                if port.service and (not current.service or current.service == "tcp"):
                    current.service = port.service
                if len(port.version or "") > len(current.version or ""):
                    current.version = port.version
        existing.ports.sort(key=lambda value: (value.protocol, value.port))

    def add_crypto(self, crypto_finding: CryptoFinding):
        """Add or replace a TLS finding for one endpoint."""
        for index, existing in enumerate(self.findings_crypto):
            if (existing.host, existing.port) == (crypto_finding.host, crypto_finding.port):
                self.findings_crypto[index] = crypto_finding
                return
        self.findings_crypto.append(crypto_finding)

    def add_web(self, web_result: WebAssessmentResult):
        """Add or replace a web result for one endpoint."""
        for index, existing in enumerate(self.findings_web):
            if (existing.host, existing.port) == (web_result.host, web_result.port):
                self.findings_web[index] = web_result
                return
        self.findings_web.append(web_result)

    def add_vuln(self, vuln: VulnerabilityFinding):
        """Add or replace a normalized vulnerability finding."""
        key = (vuln.host, vuln.port, vuln.template_id, vuln.url, vuln.matcher_name)
        for index, existing in enumerate(self.findings_vulns):
            existing_key = (
                existing.host, existing.port, existing.template_id,
                existing.url, existing.matcher_name,
            )
            if existing_key == key:
                self.findings_vulns[index] = vuln
                return
    ...
    def add_error(self, phase: str, module: str, error_msg: str):
        ...
        if any(
            item.phase == phase and item.module == module and item.error == error_msg
            for item in self.execution_errors
        ):
            return
