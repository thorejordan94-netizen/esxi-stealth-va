...
class Phase7Delta(PhasePlugin):
    ...
    def _get_history_dir(self, config: Dict[str, Any]) -> Path:
        ...
        configured = d_cfg.get("history_dir", "output/history")
        if configured in ("output/history", "history"):
            return get_output_dir(config) / "history"
        path = Path(configured).expanduser()
        if path.is_absolute():
            return path
        return Path(__file__).resolve().parent.parent / path

    def _find_previous_report(self, history_dir: Path,
                              output_dir: Optional[Path] = None) -> Optional[AssessmentReport]:
        """
        ...
        if not history_dir.exists():
            ...
            main_report = (output_dir or history_dir.parent) / "assessment_report.json"
    ...
    def execute(self, report: AssessmentReport, config: Dict[str, Any]):
        ...
        previous = self._find_previous_report(history_dir, output_dir)
        ...
        report.set_finished()
        self._archive_report(report, history_dir, output_dir)
    ...
    def mock_execute(self, report: AssessmentReport, config: Dict[str, Any]):
        ...
        history_dir = self._get_history_dir(config)
        self._archive_report(report, history_dir, get_output_dir(config))
