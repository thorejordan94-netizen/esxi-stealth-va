...
class Phase3Enum(PhasePlugin):
    ...
    def _get_nmap_cmd(self, config: Dict[str, Any] = None) -> List[str]:
        """Determine how to invoke nmap (native or WSL)."""
        configured = (config or {}).get("assessment", {}).get("tool_paths", {}).get("nmap", "")
        if configured and Path(configured).is_file():
            return [configured]
        native = shutil.which("nmap")
        if native:
            return [native]
        if shutil.which("wsl"):
            return ["wsl", "nmap"]
    ...
    def _deep_service_scan(self, host: str, ports: List[int],
                           ...
                           output_dir: Path,
                           config: Dict[str, Any] = None) -> List[PortEntry]:
        """
    ...
    def execute(self, report: AssessmentReport, config: Dict[str, Any]):
        ...
        for host_finding, ports in hosts_to_enum:
            ...
            enriched = self._deep_service_scan(
                ...
                stealth_cfg, version_intensity, output_dir, config
            )
