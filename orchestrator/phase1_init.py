...
from orchestrator.runtime import get_output_dir, get_privilege_prefix, run_command
...
class Phase1Init(PhasePlugin):
    ...
    def _get_tool_path(self, tool_name: str, config: Dict[str, Any]) -> str:
        ...
        tool_paths = config.get("assessment", {}).get("tool_paths", {})
    ...
    def _install_testssl_script(self, privilege_prefix: list):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            install_dir = Path("/usr/local/testssl.sh")
            install_path = install_dir / "testssl.sh"
            run_command(privilege_prefix + ["mkdir", "-p", str(install_dir)], check=True)
            run_command(privilege_prefix + ["cp", "-R", str(extracted_dir) + "/.", str(install_dir)], check=True)
            run_command(privilege_prefix + ["chmod", "+x", str(install_path)], check=True)
            run_command(privilege_prefix + ["ln", "-sf", str(install_path), "/usr/local/bin/testssl.sh"], check=True)
    ...
    def _install_nikto_script(self, privilege_prefix: list):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            install_dir = Path("/usr/local/nikto")
            install_path = install_dir / "program" / "nikto.pl"
            run_command(privilege_prefix + ["mkdir", "-p", str(install_dir)], check=True)
            run_command(privilege_prefix + ["cp", "-R", str(extracted_dir) + "/.", str(install_dir)], check=True)
            run_command(privilege_prefix + ["chmod", "+x", str(install_path)], check=True)
            run_command(privilege_prefix + ["ln", "-sf", str(install_path), "/usr/local/bin/nikto"], check=True)
    ...
    def execute(self, report: AssessmentReport, config: Dict[str, Any]):
        ...
        phases_cfg = assessment_cfg.get("phases", {})
        tools = {
            "nmap": True,        # Required
            "curl": bool(phases_cfg.get("phase5_web", True)),
            "nuclei": bool(phases_cfg.get("phase6_vulnscan", True) and assessment_cfg.get("nuclei", {}).get("enabled", True)),
        ...
        auto_install = bool(config.get("_auto_install", True))
        if missing_required:
            if dry_run:
                ...
                for tool in missing_required:
                    report.add_error("phase1_init", self.name,
                        f"Required tool '{tool}' is missing and automatic installation is disabled.")
        ...
        if missing_optional:
            if dry_run:
                ...
                logger.info("Automatic installation disabled; optional tools will be skipped.")
