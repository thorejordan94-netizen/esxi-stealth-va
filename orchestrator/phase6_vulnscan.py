    ...
    def execute(self, report: AssessmentReport, config: Dict[str, Any]):
        ...
        try:
            ...
            if output_file.exists():
                output_file.unlink()
            result = run_command(cmd, capture_output=True, text=True, timeout=3600, strip_proxy=True) # 1 hour timeout
            if result.returncode not in (0, 1):
                logger.warning("Nuclei exited with code %s: %s", result.returncode, (result.stderr or "")[:500])
